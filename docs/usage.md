...
Comprehensive reference. For the quick pitch and a 60-second quickstart, see the [README](../README.md).
...
- [Token auth scheme (v1 vs v2)](#token-auth-scheme-v1-vs-v2)
- [Output formats](#output-formats)
- [Columns](#columns)
- [Search](#search)
- [Pager](#pager)
- [Plugin passthrough](#plugin-passthrough)
- [TUI](#tui)
...
API filters AND presentation modifiers are **positional keyword/value pairs**, Junos-style. Flags are reserved for session/connection setup — things you rarely change per-invocation.

```sh
nbcli show sites
nbcli show sites status active
nbcli show sites region us-west status active limit 100
nbcli show sites name hq format json                  # presentation positional
nbcli show sites columns id,name,status format yaml   # both modifiers
nbcli show sites limit 0 format json                  # stream full inventory
```

Trailing positional grammar every `show` + `search` command supports:

| Keyword | Shape | Notes |
|---|---|---|
| `limit <N>` | value | page size (`0` = stream all pages) |
| `offset <N>` | value | page offset |
| `format <fmt>` | value | `table` \| `json` \| `yaml` \| `tsv` |
| `columns <list>` | value | comma-separated, names from `nbcli columns <resource>` |
| `pager` | switch | open the less-like interactive pager |
...
Error: unknown keyword "foo" (expected one of: columns, format, limit, name, offset, pager, region, slug, status, tenant)
```

Session-level flags (set once, rarely changed): `--url`, `--config`, `--env-file`, `--auth-scheme`, `--insecure`, `--timeout`, `--verbose`.
...
1. positional presentation keyword on the command (e.g. `format json`, `columns id,name`)
2. flag — session-level only; no flag aliases for `format`/`columns`
3. env: `NBCLI_*` (e.g. `NBCLI_URL`, `NBCLI_FORMAT`)
4. config file: `$XDG_CONFIG_HOME/nbcli/config.yaml` or `~/.config/nbcli/config.yaml`
5. built-in defaults
...
Override per-call: `nbcli show sites format json` (positional, not a flag).

`limit 0` enables streaming: rows write as they arrive when the format supports it (json / yaml / tsv). Memory stays O(1) and output begins immediately. `format table` always buffers because it needs all rows to align columns.
...
Per-call override (positional):

```sh
nbcli show devices columns id,name,primary_ip4,serial
nbcli show sites columns id,name,status format json
```
...
```sh
nbcli search sites hq                 # one resource, REST ?q=
nbcli search ip-addresses 10.0.0
nbcli search vrfs prod limit 200
nbcli search all hq                   # cross-resource (fan-out across every typed endpoint)
nbcli search all hq pager             # interactive pager
```

`search <module> <key>` uses the same column set as `show <module>`. `search all <key>` fans out across every typed REST list endpoint in parallel with `?q=<key>`, then merges the hits. (Netbox 4.x GraphQL doesn't expose a `q` free-text filter — only per-field operators like `i_contains` — so REST `?q=` is the only path to true cross-field search.) Results render in a five-column view:

```
TYPE                FIELD       VALUE          DISPLAY        ATTRIBUTES
tenancy.contact     name        Ravi Pina      Ravi Pina      title=SRE, email=ravi@cow.org
virtualization.vm   comments    owner: ravi    i-deadbeef     site=hq, cluster=prod-1, tenant=Acme
ipam.ipaddress      description ravi mgmt      10.0.0.1/24    vrf=mgmt, status=Active, tenant=Acme
```

`FIELD` / `VALUE` are detected client-side from the returned row (Netbox's typed endpoints don't tell us which attribute matched, so we scan). `ATTRIBUTES` mirrors the Netbox web-UI global-search Attributes column — per-type context (site/cluster/tenant for VMs, title/email for Contacts, etc.).

## Pager

`pager` is a positional switch on every `show` / `search` command:

```sh
nbcli show ip-addresses pager
nbcli show ip-addresses status active pager
nbcli search all hq pager
```

Less-like keybinds: `n` / `space` / `enter` next, `p` / `⌫` prev, `<` / `>` first/last, `g` goto page, `/` search, `c` clear search, `?` help, `q` / `Esc` quit.
...
`nbcli tui` opens a bubbletea shell that mirrors the Netbox web UI. Tenants and Contacts render live tables; other items are placeholders.
...
| `C` | Open the column picker popup |
...
- positional keywords per resource, with already-typed ones filtered out (`nbcli show sites <TAB>` → `columns format limit name offset pager region slug status tenant`)
- static value enums (`nbcli show sites status <TAB>` → `active planned staging decommissioning retired`, `format <TAB>` → `table json yaml tsv`)
- switch keywords like `pager` (advance the cursor without expecting a value)
- session flags (`--url`, `--verbose`, …)
...
internal/cmdutils/        # Junos-style positional keyword parser + helpers
